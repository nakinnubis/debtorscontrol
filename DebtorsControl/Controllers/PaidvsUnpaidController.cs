    ...
    public class PaidvsUnpaidController : ApiController
    {
        ...
        public IEnumerable<Activiti> Get(string month)
        {
         //   return new string[] { "value1", "value2" };
            ...
            {
                var paid = db.Nairas.Count(c => c.Outstanding == 0 && c.Month.Equals(m));
                var unpaid = db.Nairas.Count(c => c.Outstanding != 0 && c.Month.Equals(m));
    ...
    public class NairaDebtController : ApiController
    {
        public IEnumerable<Debts> Get()
        {
            ...
            using (pdInvoiceEntities db = new pdInvoiceEntities())
            {
                ...
                var data = db.Nairas.Where(c => c.Year == year).ToList();
                foreach (var b in m)
                {
                    var ninflow =data.Where(c => c.Month == b).Sum(c => c.AmountPaid);
                    var outflow = data.Where(c => c.Month == b).Sum(c => c.Outstanding);
                    var p = new Debts
                    {
                      //  Dtype = "Paid",
    ...
    public class DollarDebtController : ApiController
    {
        public IEnumerable<Debts> Get()
        {
            ...
            using (pdInvoiceEntities db = new pdInvoiceEntities())
            {
              ...
              //  var months = db.Nairas.Select(c => c.Month).Distinct().AsEnumerable();
                var year = DateTime.Now.Year;
                var data = db.Dollars.Where(c => c.Year == year).ToList();
                foreach (var b in m)
                {
                    var ninflow = data.Where(c => c.Month == b).Sum(c => c.AmountPaid);
                    var outflow = data.Where(c => c.Month == b).Sum(c => c.Outstanding);
    ...
    public class DebtDollarClientController : ApiController
    {
        public IEnumerable<Debts> Get(string yr, string client)
        {
            ...
            using (pdInvoiceEntities db = new pdInvoiceEntities())
            {
                ...
                var data = db.Dollars.Where(c => c.Year == queryYear && c.ClientName.Equals(client)).ToList();
                foreach (var b in m)
                {
                    var ninflow = data.Where(c => c.Month == b).Sum(c => c.AmountPaid);
                    var outflow = data.Where(c => c.Month == b).Sum(c => c.Outstanding);
    ...
    public class DebtNairaClientController : ApiController
    {
        public IEnumerable<Debts> Get(string yr, string client)
        {
            ...
            using (pdInvoiceEntities db = new pdInvoiceEntities())
            {
                ...
                var year = DateTime.Now.Year;
                var queryYear = int.Parse(yr);
                var data = db.Nairas.Where(c => c.Year == queryYear && c.ClientName.Equals(client)).ToList();
                foreach (var b in m)
                {
                    var ninflow = data.Where(c => c.Month == b).Sum(c => c.AmountPaid);
                    var outflow = data.Where(c => c.Month == b).Sum(c => c.Outstanding);
